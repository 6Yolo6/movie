...
export default function RegisterPage(){
 ...
 return <div className="flex min-h-[80vh] items-center justify-center px-4"><Card title="注册 GYing Movie" className="w-full max-w-md shadow-xl">
  ...
  <Form form={form} onFinish={submit} size="large" layout="vertical">
   ...
   {policy?.emailVerificationEnabled&&<Form.Item label="邮箱验证码"><Space.Compact className="w-full"><Form.Item name="emailCode" noStyle rules={[{required:true,message:"请输入邮箱验证码"}]}><Input prefix={<SafetyCertificateOutlined/>}/></Form.Item><Button loading={sending} onClick={sendCode}>发送验证码</Button></Space.Compact></Form.Item>}
