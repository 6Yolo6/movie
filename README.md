...
- 我的投稿：用户可查看自己的资源投稿、审核状态、链接健康状态和举报数，并可编辑或删除自己的投稿
...
- P1 投稿编辑：新增 `PUT /api/resources/{id}`，用户可编辑自己的投稿，是否重新审核跟随全局“资源审核”开关；详情页会置顶自己的投稿并提供编辑入口
- P1 失效举报确认：详情页举报失效改为二次确认，提供取消按钮避免误点
...
- `PUT /api/resources/{id}`：编辑自己的资源投稿，普通用户按全局审核配置决定是否重新审核
