...
## 7. 迅雷状态文件写入边界（2026-09-25 复核）

- 在线 `/app/data/xunlei-auth.json` 已观测为 `10001:10001 / 600`（最新 mtime 2026-09-25 10:33:36）；backend:20260924c 的 jar 已包含 `PrivateFileWriter`。同步任务 09:27、10:33 均返回 0，两次 stat 均为 600；这不等于已验证 backend 自身的独立重复写入路径或实际授权有效性。
...
- 已增加重复写入、旧 `644` 替换、符号链接和失败清理回归；补丁已经部署，无需重复发布。后续生产重复写入验收前仍须备份状态文件及旧镜像，只检查 owner/mode 和授权状态，不打印内容。
